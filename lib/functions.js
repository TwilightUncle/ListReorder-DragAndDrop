...
function ReorderListDragAndDrop(options) {
    ...
    if (!options) {
        console.warn(ReorderListDragAndDrop.name, '引数がありません');
        return false;
    }
    if (!options.target || (typeof options.target !== 'string')) {
        console.warn(ReorderListDragAndDrop.name, '引数のtargetプロパティは必須です。文字列で指定してください');
        return false;
    }
    ...
    window.addEventListener('DOMContentLoaded', function () {
        const list_parents          = document.getElementsByClassName(target);
        parent_elem                 = list_parents[0];
        parent_elem.style.display   = 'block';
        const elems                 = parent_elem.children;
    ...
    function itemsInit() {
        // そろえ位置を取得する要素
        const element   = (list_elements[0]) ? list_elements[0] : parent_elem;
    ...
    function mouseDown(e) {
        ...
        // 要素がクリックされた座標を保持
        click_x                 = e.pageX - drag_element.offsetLeft;
        click_y                 = e.pageY - drag_element.offsetTop;
    ...
    function mouseMove(e) {
        ...
        //マウスの場所に要素を動かす
        drag_element.style.top     = e.pageY - click_y + "px";
        drag_element.style.left    = e.pageX - click_x + "px";
    ...
    function reOrderForMoveItems() {
        ...
        const direct1         = (direction === 'vertical') ? 'height'         : 'width';
        const direct2         = (direction !== 'vertical') ? 'height'         : 'width';
        const get_size2       = (direction !== 'vertical') ? 'offsetHeight'   : 'offsetWidth';
